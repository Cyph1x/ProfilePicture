...
step_count = 100 # Number of steps
...
def generate_mice_lines(n: int, n_steps: int, distance: float, rotation: float, reverse: bool) -> Group:
    ...
    for i in range(n_steps):
        ...
        mouse_lines = draw.Lines(*mouse_lines, close=True, fill='none')
        target_lines = draw.Lines(*target_lines, close=True, fill='none')

        if reverse:
            if enable_animation:
                mouse_lines.append_anim(draw.Animate(attributeName='d', to=target_lines.args["d"], begin=f"{begin_ms}ms", dur=f"{animation_duration_ms}ms",repeatCount='indefinite'))
            ...
            if enable_animation:
                target_lines.append_anim(draw.Animate(attributeName='d', to=mouse_lines.args["d"], begin=f"{begin_ms}ms", dur=f"{animation_duration_ms}ms",repeatCount='indefinite'))
            ...
            lines = target_lines
        blank_lines = deepcopy(lines)
        lines = deepcopy(lines)
        lines.args["stroke"] = "white"
        lines.args["stroke_width"] = 3
        g.append(lines)

        g.append(lines)
        lines = deepcopy(blank_lines)
        lines.args["stroke"] = "red"
        lines.args["stroke_width"] = 0.1
        g.append(lines)
        for mouse in new_mice:
